...
边缘触发下对于非阻塞write的操作，需要反复写入直到写完或者EAGAIN为止,这里存在一个问题，如果读到EAGAIN后就直接跳出那么就会有一部分数据没有发送出去，有两种做法，一种是等待一定的时间后重新尝试，另一种是将未发送出去的数据记录下来，并在epoll中注册对可读的监听，等待此套接字的发送缓冲区中有位置后再次尝试write  
```c++
                char  line[MAXLINE], result[MAXN];
		//result中存入需要发送的数据
